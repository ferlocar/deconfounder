...
cdef class DeconfoundCriterion(Criterion):
    ...
    cdef BoundaryRecord init_boundary(self, double max_score, double[2] sum_arr, double[2] weighted_n_arr) nogil:
        ...
        br.reward = sum_arr[U_IX] / (1 - br.p_t)
        br.tmf = weighted_n_arr[U_IX] / (1 - br.p_t)
        return br
    ...
    cdef BoundaryRecord node_decision_boundary(self) nogil:
        ...
        for p in range(start, end):
            ...
            if (curr.threshold > scores[i]):      
                if (curr.reward / curr.tmf) > (best.reward / best.tmf):
                    best = curr
            ...
            if self.treated[i]:
                ...
                curr.reward -= w * y_i / (1 - curr.p_t)
                curr.tmf -= w / (1 - curr.p_t)
    ...
    cdef BoundaryRecord* children_decision_boundary(self) nogil:
        ...
        for p in range(start, end):
            ...
            if (curr[is_left].threshold > scores[i]):      
                if (curr[is_left].reward / curr[is_left].tmf) > (best[is_left].reward / best[is_left].tmf):
            ...
            if self.treated[i]:
                ...
                curr[is_left].reward -= w * y_i / (1 - curr[is_left].p_t)
                curr[is_left].tmf -= w / (1 - curr[is_left].p_t)
    ...
    cdef double node_impurity(self) nogil:
        ...
        for k in range(self.n_outputs):
            ...
            impurity -= boundary.reward / boundary.tmf
    ...
    cdef void children_impurity(self, double* impurity_left,
        ...
        if self.weighted_n_left_arr[U_IX]==0 or self.weighted_n_left_arr[T_IX]==0 \
            ...
            for k in range(self.n_outputs):
                ...
                impurity_left[0] -= children_boundary[L_IX].reward / (children_boundary[L_IX].tmf)
                impurity_right[0] -= children_boundary[R_IX].reward / (children_boundary[R_IX].tmf)
